<footer class="bg-white dark:bg-neutral-900">
    <div class="px-4 py-12 mx-auto max-w-7xl sm:px-6 md:flex md:items-center md:justify-between lg:px-8">
        <div class="flex space-x-6 md:order-2 text-sm">
            <a href="/terms-of-service" class="text-gray-600 hover:opacity-70 dark:text-white">Terms of Service</a>
            <a href="/privacy-policy" class="text-gray-600 hover:opacity-70 dark:text-white">Privacy Policy</a>
        </div>
        <div class="flex justify-center space-x-6 md:order-2">
            <span class="inline-flex justify-center w-full gap-3 lg:ml-auto md:justify-start md:w-auto">
                <a href="https://github.com/abinantonyarackapparampil" class="w-6 h-6 transition fill-black hover:text-blue-500 dark:fill-neutral-400 dark:hover:text-blue-500">
                    <span class="sr-only">github</span>
                    <x-icon-github class="w-5 h-5 md hydrated" aria-label="logo github" />
                </a>
                <a href="https://twitter.com/_abinantonyarackapparampil" class="w-6 h-6 transition fill-black hover:text-blue-500 dark:fill-neutral-400 dark:hover:text-blue-500">
                    <span class="sr-only">twitter</span>
                    <x-icon-twitter class="w-5 h-5 md hydrated" aria-label="logo twitter" />
                </a>
                <a href="https://linkedin.com/in/abinantonyarackapparampil" class="w-6 h-6 transition fill-black hover:text-blue-500 dark:fill-neutral-400 dark:hover:text-blue-500">
                    <span class="sr-only">Linkedin</span>
                    <x-icon-linkedin class="w-5 h-5 md hydrated" role="img" aria-label="logo linkedin" />
                </a>
            </span>
        </div>
        <div class="mt-8 md:mt-0 md:order-1">
            <p class="text-base text-center text-neutral-400 dark:text-neutral-600">
                <span class="mx-auto mt-2 text-sm text-neutral-500 dark:text-white">
                    Copyright © 2023 by -
                    <a href="https://edgesys.tech" class="text-blue-500 hover:underline dark:text-blue-500 dark:hover:text-blue-500" rel="noopener noreferrer">
                        @edgesys.tech
                    </a>
                </span>
            </p>
        </div>
    </div>
</footer>